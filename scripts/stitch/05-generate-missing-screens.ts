...
import { BRIEFS_MISSING } from './lib/briefs-missing.js';
...
async function main() {
  ...
  for (const brief of BRIEFS_MISSING) {
    ...
    try {
      ...
      writeJson('05-generated-missing-screens.json', records);
    } catch (err) {
  ...
  console.log(`\n[stitch] generated ${ok}/${records.length} missing screens`);
  if (ok === 0) {
    process.exit(2);
  }
}
...
main().catch((err) => {
  console.error('[stitch] generate-missing FAILED');
  console.error(err);
  process.exit(1);
});
