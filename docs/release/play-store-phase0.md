...
| PostHog (shared project) | **HUMAN** | 1차 merge: `ENABLED=false` · prod ON 시 key + `true` |
...
5. **개인정보처리방침·이용약관 URL** (프로덕션 API, **privacy v4**·PostHog 위탁 반영 후):
   ...
   - 원문: [`docs/legal/terms.md`](../legal/terms.md), [`docs/legal/privacy.md`](../legal/privacy.md) — DB 갱신: 마이그레이션 `20260530120000_policy_posthog_v4` (privacy v4)
...
| `EXPO_PUBLIC_POSTHOG_KEY` | (선택) 기존 PostHog shared Project API key (`phc_...`) |
| `EXPO_PUBLIC_POSTHOG_HOST` | `https://us.i.posthog.com` |
| `EXPO_PUBLIC_POSTHOG_ENABLED` | **`false`** (테스터·1차 merge 기본) → 분석 ON 시 `true` + production 재빌드 |

PostHog는 **shared Project 1개**를 다른 서비스와 공유합니다. 대시보드 필터: `app = nourylog`, `environment = production`.  
운영·월간 점검·대시보드 초안: [`docs/qa/posthog-monthly-review.md`](../qa/posthog-monthly-review.md)

```powershell
cd apps/mobile
npx eas-cli secret:create --scope project --name EXPO_PUBLIC_API_URL --value "https://api-server-production-52bc.up.railway.app"
# 분석 켤 때 (prod 빌드 직전):
# npx eas-cli secret:create --scope project --name EXPO_PUBLIC_POSTHOG_KEY --value "phc_..."
# npx eas-cli secret:create --scope project --name EXPO_PUBLIC_POSTHOG_HOST --value "https://us.i.posthog.com"
# npx eas-cli secret:create --scope project --name EXPO_PUBLIC_POSTHOG_ENABLED --value "true"
```
...
- `apps/mobile/src/analytics/`: PostHog HTTP 연동 (`app=nourylog`, `EXPO_PUBLIC_POSTHOG_ENABLED` 기본 `false`)
...
1. 서버 `npm run build -w @diet-management/server` · `prisma migrate deploy` (프로덕션: `20260530120000_policy_posthog_v4` 포함)
...
- 스토어 등록정보·스크린샷·Data safety 설문 (PostHog 분석 SDK — prod ON·privacy v4와 동시 갱신)
