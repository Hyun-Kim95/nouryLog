...
|---|---|---|---|
| 기본(신규, 이력 없음) | Segmented: `g` + 기본 칩 | `g`면 숨김. 다른 단위면 **표시·필수** | `grams = 수량 × 1단위g`. 단위≠g이면 `PORTION_COUNT`+`portionLabel` 스냅샷(템플릿 없으면 foodTemplateId 없음) |
| g → 미환산 단위 (예: 개) | 「개」선택 | **현재 총 g를 1단위=g로 채움**, 수량은 **1** (120g→개 = 1개·120g/개. **120개로 재해석 금지**) | 총 grams 유지 |
...
| 항목 | 스펙 |
|---|---|
| PORTION_COUNT + 템플릿 | 표시 `N{단위}`(예: `2개`). −/+ **±1 단위**. 모달은 분량 수(0.1 허용) |
| 같은 이름 템플릿·이력으로 1단위=g를 알 때 | 표시 `N{단위}`. −/+ **±1 단위**(저장은 grams 스케일). 모달은 분량 수 |
| 수기 스냅샷(템플릿 없음, `portionLabel`) | 표시 `N{단위}`. −/+ **±1 단위**(grams 스케일, 수량·라벨 유지) |
...
| 0.8 | 2026-08-11 | g→개 전환 시 총 g를 1단위로 등록(수량 1). 숫자 재해석 금지 |
| 0.9 | 2026-08-17 | 최근/제안 탭 시 섭취량·단위도 폼에 채움 (PORTION은 개수+단위, 그 외 g) |
| 0.10 | 2026-08-17 | 목록: 같은 이름 템플릿 배수면 N개 표시·±1 단위. 모달은 0.1 유지 |
| 0.11 | 2026-08-17 | 신규 수기 PORTION: `portionLabel` 스냅샷. 목록도 단위 ±1 |
