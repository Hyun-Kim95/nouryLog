...
| `week` | 앵커 주 **7일** 일별 전개 | **6주** 주간 합계 버킷, `daily[]` **6포인트** |
| `month` | 앵커 월 **전체 일** 일별 전개 | **6개월** 월간 합계 버킷, `daily[]` **6포인트** |
...
- `summary`, `byMealSlot`: 윈도우 합계 → **기록 있는 버킷 수**로 일평균.

### `daily[]` 항목

| 필드 | 설명 |
|------|------|
| `date` | 버킷 키: `YYYY-MM-DD`(일·주 월요일) 또는 `YYYY-MM-01`(월) |
...
- `range=meal` 변경 없음.
- `goalAchievement` 필드 유지(버킷 기준 집계).
