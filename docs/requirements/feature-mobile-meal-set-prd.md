...
- (D6, 2026-06-29 실기기 피드백 반영, 프론트 전용):
  - 세트는 **끼니를 고정하지 않는다.** 편집 화면에서 기본 끼니/간식 위치 입력 제거, **끼니는 등록(apply) 시 현재 시간 기준 기본값으로 제안 후 선택**한다. (서버 계약 호환 위해 `defaultMealSlot`은 안전 placeholder만 전송)
  - 항목 추가 모달에 **인라인 "담음 ✓ n" 표시**(토스트가 모달에 가려지는 문제 대응) + 키보드가 검색 입력을 가리지 않도록 `KeyboardAvoidingView` 적용.
  - 항목/세트/등록 미리보기에 **탄단지(g) 함께 표시**(기존 칼로리만 → 탄·단·지 추가).
- (D6-2, 2026-06-29 실기기 2차 피드백 반영, 프론트 전용 / API 계약·apply 로직 불변):
  - **개수(인분/배수) 모델:** 동일 음식은 항목 1개로 합치고 **개수 증감(− n +)**으로 양을 조절한다. 등록 시 **음식당 식사 1건**(템플릿=인분 수 `PORTION_COUNT`, 수기=영양×배수 단일 항목)으로 생성한다. 과거 기록을 담을 때 정확 그램은 유지하지 않고 **인분 기반으로 단순화**(레거시 `TOTAL_GRAMS` 항목은 로드 시 `servingGrams`로 인분 환산).
  - **통합 검색:** 추가 모달의 탭(최근/검색)·분량 입력을 제거하고 **단일 검색창**으로 통일한다. 검색어가 없으면 최근 기록(`/meals`), 있으면 `/me/meal-entry-suggestions`(템플릿+과거기록 혼합)로 **과거 기록도 검색**된다. 각 결과 행에 kcal·탄단지·담은 개수 표시 + 담기/스테퍼.
  - **세트 내용 노출:** 세트 목록 카드에 포함 음식명(앞 4개 + "외 N개")을 표시한다.
  - **몸무게 등록 키보드:** `WeightCheckInModal`의 Android `KeyboardAvoidingView` behavior를 `height`로 수정해 입력칸 가림 해소.
