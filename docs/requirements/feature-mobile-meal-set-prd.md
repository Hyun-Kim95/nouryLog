...
### 6.2 후순위 범위 (선택, 이번 범위 아님)
- **수동 항목**(템플릿에 없는 음식명 + 영양 4종 + 그램 스냅샷) 지원(D2: MVP 제외, 후속 확장).
- 지난 끼니를 그대로 복제해 세트로 저장(과거 식사 → 세트 변환).
...
- (D2) **MVP는 템플릿 항목만** 지원. 수동 항목은 후순위(§6.2). 데이터 모델은 `kind` 분기로 확장 여지만 유지.
